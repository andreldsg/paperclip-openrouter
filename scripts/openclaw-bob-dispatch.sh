...
handover_file="${1:-}"
log_file="${OPENCLAW_BOB_DISPATCH_LOG:-/tmp/openclaw-bob-dispatch.log}"
...
{
  echo "OPENCLAW_BOB_DISPATCH_STARTED"
  date -Is
  echo "cwd_before=$(pwd)"
  echo "user=$(whoami)"
...
{
  echo "OPENCLAW_BOB_DISPATCH_EXIT_STATUS=$status"
  date -Is
  echo "OPENCLAW_BOB_DISPATCH_FINISHED"
} >> "$log_file"
